...
# %% fit the training set to a gamma distribution to get the gamma params
alpha, theta = utl.gamma_unbiased_estimator(train['price'].values)
...
class ebm_bert_model(torch.nn.Module):
    ...
    def __init__(self, transformer, category_embedding_dim, brand_embedding_dim,
                 ...
                 hidden_dims, gamma_params, target_range, device,
        ...
        self.learned_target_pdf_layer = lyr.Gamma_Gaussian_Mixture_EBM_Sampler(
            self.vector_dim, self.hidden_dims, gamma_params, target_range, device,
            normal_prob=normal_prob, normal_sigma_divider=normal_sigma_divider,
            uniform_prob=uniform_prob, uniform_range=uniform_range,
    ...
    def forward(self, encoded_name, encoded_description, 
        ...
        x = self.initial_vec_forward(encoded_name, encoded_description,
                                     category_name, brand_name, x_numeric)
        y, y_array, q_array = self.learned_target_pdf_layer(x, t)
...
def ebm_importance_eval(model, loss_function, dataloader, device):
    ...
    for data in dataloader:
        with torch.no_grad():
            y, y_array, q_array =\
                model(t=data['target'], **data['features'])
...
model = ebm_bert_model(
    ...
    normal_sigma_divider=normal_sigma_divider,
...
for epoch in range(num_epochs):
    ...
    eval_epoch_loss = ebm_importance_eval(model, loss_function, val_dataloader, device)
...
#training time: 13066.969418287277 seconds
...
model.learned_target_pdf_layer.t_range = model.t_range = torch.arange(0, 1600.5, 0.5).to(device)


p, p_dist, t, t_ev = learned_pdf_score(model, val_dataset, batch_fraction=1)

print(r2_score(t, t_ev)) #0.5412901824674571
print(rmsle(t, t_ev)) #0.44518160140185803

# %% get the mode
t_range=model.t_range.detach().cpu().numpy()
t_mode = t_range[np.argmax(p_dist, axis=1)]

print(r2_score(t, t_mode))  #0.4660318444633661
print(rmsle(t, t_mode)) #0.4782646001359596
...
def plot_dist(p_dist, t=None, tev=None, t_range=model.t_range.detach().cpu().numpy()):
    ...
    if t:
        plt.axvline(x=t, color='b', label='target')
    
    if tev:
        plt.axvline(x=tev, color='r', label='ev')
...
scored_sample = dict({'target': t[idx], 'mean_pred': t_ev[idx], 'mode_pred': t_mode[idx]})
pprint(scored_sample)

true_val = scored_sample['target']
ev = scored_sample['mean_pred']

plot_dist(p_dist[idx], t=t[idx], tev=t_ev[idx])
